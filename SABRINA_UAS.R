...
server <- function(input, output) {
  ...
  observeEvent(input$predict_button, {
    ...
    output$Monthly_Sales_Volume <- renderPlot({
      ...
      points(new_data$x1, predicted_Monthly_Sales_Volume, col = "scarlet", pch = 16)
      legend("topright", legend = c("Actual Data", "Predicted Data"), col = c("#3498DB", "scarlet"), pch = c(1, 16))
    })
